...
# MAIN DIRECTORY
currentDirectory = os.path.join(os.getcwd(),'Assets')

# INITIALIZE SCREEN
screen = getScreen()
 
# WINDOW
windowIcon = pygame.image.load(os.path.join(currentDirectory,'Icon.png')).convert_alpha()
...
# STAGE WIPE CLOUD
stageCloudImg = pygame.image.load(os.path.join(currentDirectory,'StageCloud.png') ) .convert_alpha()
...
for filename in os.listdir(explosionDirectory):
    if filename.endswith('.png'):
        ...
        explosionList.append(pygame.image.load(path).convert_alpha())
...
for levelFolder in os.listdir(shipDirectory):
    ...
    if os.path.isdir(levelFolderPath): # Ignore DS_STORE on MacOS
        shipLevelList = []
        for shipAsset in os.listdir(levelFolderPath): # Ship assets per level
            ...
            if os.path.isdir(shipAssetPath): # Ignore DS_STORE on MacOS
                assetList = []
                for imageAsset in os.listdir(shipAssetPath): # Iterate through image folders
                    
                    if imageAsset.endswith('.png'): 
                        imageAssetPath = os.path.join(shipAssetPath,imageAsset)
                        imageAssetPng = pygame.image.load(imageAssetPath)
                ...
                laserPng = pygame.image.load(shipAssetPath)
                laserPng.set_colorkey([255,255,255]) 
                shipLevelList.append(laserPng.convert_alpha())

        #try: shipLevelList = shipLevelList.sort()
        #except: continue
...
# [   ( [Exhaust frames],[Ship Skins],laser image,{Player Constants} )   ]

# MAIN MENU ASSETS
menuList = []
menuList.append(pygame.image.load(os.path.join(menuDirectory,'A.png')).convert_alpha())
menuList.append(pygame.image.load(os.path.join(menuDirectory,'O.png')).convert_alpha())
menuList.append(pygame.image.load(os.path.join(menuDirectory,'big.png')).convert_alpha()) 
menuList.append(pygame.image.load(os.path.join(menuDirectory,'left.png')).convert_alpha())
menuList.append(pygame.image.load(os.path.join(menuDirectory,'right.png')).convert_alpha())
menuList.append(pygame.image.load(os.path.join(menuDirectory,'dblue.png')).convert_alpha())
menuList.append(pygame.image.load(os.path.join(menuDirectory,'lblue.png')).convert_alpha())
menuList.append(pygame.image.load(os.path.join(menuDirectory,'lgreen.png')).convert_alpha())
menuList.append(pygame.image.load(os.path.join(menuDirectory,'dgreen.png')).convert_alpha())
menuList.append(pygame.image.load(os.path.join(menuDirectory,'orange.png')).convert_alpha())
menuList.append(pygame.image.load(os.path.join(menuDirectory,'red.png')).convert_alpha())
menuList.append(pygame.image.load(os.path.join(menuDirectory,'white.png')).convert_alpha())
menuList.append(pygame.image.load(os.path.join(menuDirectory,'yellow.png')).convert_alpha())
...
class Menu:
    ...
    def home(self,game,player):
        ...
        if game.savedOverallHighScore >= 330: game.unlockNumber = len(spaceShipList[0][1])
        elif game.savedOverallHighScore >= 300: game.unlockNumber = len(spaceShipList[0][1]) - 1
        elif game.savedOverallHighScore >= 270: game.unlockNumber = len(spaceShipList[0][1]) - 2
        elif game.savedOverallHighScore >= 240: game.unlockNumber = len(spaceShipList[0][1]) - 3
        elif game.savedOverallHighScore >= 210: game.unlockNumber = len(spaceShipList[0][1]) - 4
        elif game.savedOverallHighScore >= 180: game.unlockNumber = len(spaceShipList[0][1]) - 5    
        elif game.savedOverallHighScore >= 150: game.unlockNumber = len(spaceShipList[0][1]) - 6 
        elif game.savedOverallHighScore >= 120: game.unlockNumber = len(spaceShipList[0][1]) - 7   
        elif game.savedOverallHighScore >= 90: game.unlockNumber = len(spaceShipList[0][1]) - 8    
        elif game.savedOverallHighScore >= 60: game.unlockNumber = len(spaceShipList[0][1]) - 9
        elif game.savedOverallHighScore >= 30: game.unlockNumber = len(spaceShipList[0][1]) - 10
        ...
        while game.mainMenu:   
            ...
            if game.savedOverallHighScore >= 30 and len(spaceShipList[game.savedShipLevel][1]) > 1: screen.blit(skinHelpDisplay,skinHelpRect)
...
class Player(pygame.sprite.Sprite):
        def __init__(self,game):
            ...
            self.laserImage = spaceShipList[game.savedShipLevel][2]
        ...
        def lastSpaceShip(self):
            if self.currentImageNum >= 1: 
                ...
                if game.savedShipLevel == 0 and game.unlockNumber == 0:
                    self.image = spaceShipList[0][1][game.unlockNumber]
                    self.currentImageNum = game.unlockNumber
                elif game.savedShipLevel == 0 and game.unlockNumber > 0:
                    self.image = spaceShipList[0][1][game.unlockNumber-1]
                    self.currentImageNum = game.unlockNumber-1
                else:
                    self.image = spaceShipList[game.savedShipLevel][1][len(spaceShipList[game.savedShipLevel][1]) - 1]
                    self.currentImageNum = len(spaceShipList[game.savedShipLevel][1]) - 1
        ...
        def explode(self,game,obstacles):
            while self.explosionState < len(explosionList):
                ...
                for obs in obstacles:
                    newBlit = rotateImage(obs.image,obs.rect,obs.angle) # show obstacles during explosion
                    screen.blit(newBlit[0],newBlit[1])
